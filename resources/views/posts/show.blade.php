...
@section('content')
    ...
    @can('is_admin',$post)
        <section class="mt-8 mx-16">
            <div class="relative py-4">
                ...
                <div class="relative z-0 p-2 ml-12  w-1/2 group">
                ...
                <form action="{{ route('posts.status.update', ['post' => $post->id]) }}" method="post">
                    @csrf
                    <div class="relative z-0 p-2 w-1/2 ml-12  group">
                    ...
                    <div class="relative z-0 pl-2 ml-12 w-1/2 group">
                    @if($post->status == "Progress")
                        <div id="r-main" hidden>
                            ...
                            <select name="reason" id="reason" onchange="enablereason(this)"
                                    class="w-2/3 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                <option value="" selected disabled>-</option>
                                <option value="เงินที่ต้องจ่ายทั้งหมดในข้อเสนอไม่เหมาะสม">
                                    เงินที่ต้องจ่ายทั้งหมดในข้อเสนอไม่เหมาะสม
                                </option>
                                <option value="เงินมัดจำน้อยกว่าครึ่งหนึ่งของเงินที่ต้องจ่ายทั้งหมดในข้อเสนอ">เงินมัดจำน้อยกว่าครึ่งหนึ่งของเงินที่ต้องจ่ายทั้งหมดในข้อเสนอ</option>
                        ...
                        <div class="r-none" id="r-none" hidden>
                            <label for="reason_etc"
                                   class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                                เหตุผลเพิ่มเติม
                            </label>
                            <input type="text" name="reason_etc" id="reason_etc" value="{{old('reason_etc')}}"
                                   maxlength="40"
                                   class="bg-gray-50 border @error('reason_etc') border-red-600 @else border-gray-300 @enderror text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-1/2 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        </div>
                            @elseif(!is_null($post->reason) && $post->status == "Completed" || $post->status == "Cancel")
                                ...
                                <input type="text" name="reason" id="reason" value="{{ $post->reason }}" disabled
                                       class="bg-gray-50 w-2/3 border  @error('reason') border-red-600 @else border-gray-300 @enderror text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            @endif
                        ...
                        @if(!($post->status == "Completed" || $post->status == "Cancel"))
                            <div class="inline-flex pt-3">
                                <button class="app-button px-3  inline-flex" type="submit">
    ...
    @can('update', $post)
        <section class="mt-8 mx-8 mb-6">
            <div class="relative py-4">
                <div class="absolute inset-0 flex items-center">
                    <div class="w-2/3 border-b border-gray-300"></div>
                </div>
                <!-- <div class="relative flex justify-center">
